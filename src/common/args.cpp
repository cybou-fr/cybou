...
// Keep the inherited symbol name during the staged migration, but never use a
// Bitcoin Core configuration file for CYBOU.
const char * const BITCOIN_CONF_FILENAME = "cybou.conf";
...
{
    // Windows: C:\Users\Username\AppData\Local\CYBOU
    // macOS: ~/Library/Application Support/CYBOU
    // Unix-like: ~/.cybou
#ifdef WIN32
    // Never probe or reuse either legacy Bitcoin Core data directory.
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "CYBOU";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/CYBOU";
#else
    // Unix-like
    return pathRet / ".cybou";
#endif
#endif
}
