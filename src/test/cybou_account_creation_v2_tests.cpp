...
BOOST_AUTO_TEST_SUITE(cybou_account_creation_v2_tests)
...
BOOST_AUTO_TEST_CASE(hybrid_pop_and_work_bind_random_account_and_authorization)
{
    ...
    cybou::IdentityAuthorizationV2 auth{*root, *device};
    const auto commitment = cybou::ComputeIdentityAuthorizationCommitmentV2(auth);
    const auto digest = cybou::ComputeAccountCreatePopDigestV2(network_id, account_id, auth);
    ...
    for (uint64_t nonce{0}; nonce < 1000; ++nonce) {
        op.work.nonce = nonce;
        const auto hash = cybou::ComputeAccountCreateWorkHashV2(op.work);
        BOOST_REQUIRE(hash);
        if ((*hash)[0] == 0 || (*hash)[0] < 16) { mined = true; break; }
    }
    BOOST_REQUIRE(mined);
    BOOST_CHECK(cybou::ValidateAccountCreateOpV2(op, network_id, 0, params) == cybou::AccountCreateV2Error::NONE);
    const auto bytes = cybou::SerializeAccountCreateOpV2(op);
    BOOST_REQUIRE(bytes);
    ...
    BOOST_CHECK(cybou::ValidateAccountCreateOpV2(damaged, network_id, 0, params) == cybou::AccountCreateV2Error::INVALID_DEVICE_POP);
    damaged = *decoded;
