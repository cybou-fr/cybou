...
bool Valid(const IdentityAuthorizationV2& auth)
{
...
std::optional<IdentityAuthorizationBytesV2> SerializeIdentityAuthorizationV2(
    const IdentityAuthorizationV2& auth)
{
    if (!Valid(auth)) return std::nullopt;
    IdentityAuthorizationBytesV2 bytes{};
    size_t pos{0};
    bytes[pos++] = VERSION;
...
{
    if (bytes.size() != IDENTITY_AUTHORIZATION_V2_SIZE || bytes[0] != VERSION ||
        bytes[1] != ROOT_SUITE || bytes[1986] != DEVICE_SUITE) return std::nullopt;
    IdentityAuthorizationV2 auth{
...
std::optional<std::array<unsigned char, 32>> ComputeIdentityAuthorizationCommitmentV2(
    const IdentityAuthorizationV2& auth)
{
    const auto bytes = SerializeIdentityAuthorizationV2(auth);
