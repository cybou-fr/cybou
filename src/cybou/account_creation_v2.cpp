...
bool ValidSignatures(const AccountCreateOpV2& op)
{
    ...
    const AccountCreationWorkV2& work)
{
    ...
    std::array<unsigned char, ACCOUNT_CREATE_V2_WORK_SIZE> bytes{};
    bytes[0] = VERSION;
...
{
    const auto bytes = SerializeAccountCreationWorkV2(work);
    ...
    const IdentityAuthorizationV2& authorization)
{
    ...
    const auto commitment = ComputeIdentityAuthorizationCommitmentV2(authorization);
...
{
    const auto auth = SerializeIdentityAuthorizationV2(op.authorization);
    const auto work = SerializeAccountCreationWorkV2(op.work);
    if (op.account_id.IsNull() || !auth || !work || !ValidSignatures(op)) return std::nullopt;
    std::array<unsigned char, ACCOUNT_CREATE_V2_SIZE> bytes{};
    bytes[0] = VERSION;
...
{
    if (bytes.size() != ACCOUNT_CREATE_V2_SIZE || bytes[0] != VERSION) return std::nullopt;
    ...
    const auto auth = DeserializeIdentityAuthorizationV2(bytes.subspan(33, IDENTITY_AUTHORIZATION_V2_SIZE));
    if (!account_id || !auth || bytes[3364] != VERSION) return std::nullopt;
    AccountCreateOpV2 op{.account_id = *account_id, .authorization = *auth,
...
{
    if (!SerializeAccountCreateOpV2(op)) return AccountCreateV2Error::INVALID_FORMAT;
    if (op.account_id.IsNull()) return AccountCreateV2Error::NULL_ACCOUNT_ID;
    if (network_id.IsNull() || op.work.network_id != network_id) return AccountCreateV2Error::NETWORK_MISMATCH;
    if (op.work.account_id != op.account_id) return AccountCreateV2Error::ACCOUNT_ID_MISMATCH;
    const auto commitment = ComputeIdentityAuthorizationCommitmentV2(op.authorization);
    if (!commitment || op.work.authorization_commitment != *commitment) return AccountCreateV2Error::COMMITMENT_MISMATCH;
    const uint64_t epoch = EpochForHeight(block_height, params);
    if (op.work.work_epoch > epoch) return AccountCreateV2Error::FUTURE_WORK_EPOCH;
    if (epoch - op.work.work_epoch > params.account_creation_epoch_lag) return AccountCreateV2Error::EXPIRED_WORK_EPOCH;
    const auto work_hash = ComputeAccountCreateWorkHashV2(op.work);
    if (!work_hash || !HasWork(*work_hash, params.account_creation_work_bits)) return AccountCreateV2Error::INSUFFICIENT_WORK;
    const auto pop_digest = ComputeAccountCreatePopDigestV2(network_id, op.account_id, op.authorization);
    if (!pop_digest || !VerifyIdentityMessage(op.authorization.recovery_root, op.recovery_pop, *pop_digest)) {
        return AccountCreateV2Error::INVALID_RECOVERY_POP;
    }
    if (!VerifyIdentityMessage(op.authorization.initial_device, op.device_pop, *pop_digest)) {
        return AccountCreateV2Error::INVALID_DEVICE_POP;
    }
    return AccountCreateV2Error::NONE;
}
