...
- A separate native P2P session layer exchanges bounded HELLO/PING/PONG, finalized-block requests, and canonical operation submissions over persistent TCP sockets. An outbound peer manager uses the runtime's NetworkID and finalized status, tracks up to eight explicit peers, rejects duplicates and wrong-network peers, removes peers that fail health checks, and commits retrieved blocks only after canonical verification. The DEV producer exposes an optional CYP2 listener with up to eight concurrent inbound sessions; `p2p-probe`, `p2p-sync`, and `p2p-submit` exercise it. `p2p-follow` keeps a headless observer syncing from an explicit endpoint and retries transport failures. NodeRuntime owns a persistent peer session when explicitly configured, and Qt can use that path for sync and operation submission with DEV environment settings. The default bootstrap endpoint remains CYB1 until the remote CYP2 listener is deployed. Operation acknowledgments are distinct from finality. Operation gossip and block gossip remain open.
