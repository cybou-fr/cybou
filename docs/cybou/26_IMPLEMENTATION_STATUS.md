...
- A separate native P2P session layer exchanges bounded HELLO/PING/PONG frames over persistent TCP sockets. An outbound peer manager uses the runtime's NetworkID and finalized status, tracks up to eight explicit peers, rejects duplicates and wrong-network peers, and removes peers that fail health checks. The DEV producer can expose a separate single-session CYP2 listener and `p2p-probe` exercises it between two node processes. Runtime scheduling, multi-peer inbound handling, block/operation exchange, and gossip remain open.
