...
- A separate native P2P session layer exchanges bounded HELLO/PING/PONG and finalized-block request/response frames over persistent TCP sockets. An outbound peer manager uses the runtime's NetworkID and finalized status, tracks up to eight explicit peers, rejects duplicates and wrong-network peers, removes peers that fail health checks, and commits retrieved blocks only after canonical verification. The DEV producer exposes an optional single-session CYP2 listener; `p2p-probe` and `p2p-sync` exercise it between node processes. Runtime scheduling, multi-peer inbound handling, operation propagation, and gossip remain open.
