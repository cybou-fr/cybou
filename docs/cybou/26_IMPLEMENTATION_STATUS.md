...
- Four distinct PQ validator keys can be committed to a deterministic DEV genesis, but the current DEV producer remains single-validator Authority Mode. The CBS2 signing journal persists round and lock state so a restarted validator can safely resume an unfinished height; legacy CBS1 journals conservatively abstain at that height.
