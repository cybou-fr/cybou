...
- A separate native P2P session layer exchanges bounded HELLO/PING/PONG, finalized-block requests, and canonical operation submissions over persistent TCP sockets. An outbound peer manager uses the runtime's NetworkID and finalized status, tracks up to eight explicit peers, rejects duplicates and wrong-network peers, removes peers that fail health checks, and commits retrieved blocks only after canonical verification. The DEV producer exposes an optional single-session CYP2 listener; `p2p-probe`, `p2p-sync`, and `p2p-submit` exercise it. Operation acknowledgments are distinct from finality. Runtime scheduling, multi-peer inbound handling, operation gossip, and block gossip remain open.
